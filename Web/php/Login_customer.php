<?php
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST'){
        ...
        $query="select * from customer_login where email='$email_id'";
        $exist= $connect -> query($query);
        if($exist->num_rows>0){
            $query="select * from customer_login where email='$email_id' && password='$password'";
            $password_wrong=$connect -> query($query);
            if($password_wrong->num_rows<=0){
                $respose=2;//wrong password
            }
            else{
                $respose=$email_id;//correct password
            }
        }
        else{
